...
int main(int argc, char ** argv){
    ...
    *dgauss_Kernel, *hys, *edge, *cornerness;
   ...
   //  write_image_template("test.pgm", cornerness, width, height);
    ...
    printf("Number of Threads: %d Image-Height: %d Width: %d Sigma: %f End to End time: %ld computation time: %ld\n",1,height,width,atof(argv[2]),(end.tv_sec *1000000 + end.tv_usec)-(start.tv_sec * 1000000 + start.tv_usec),(computationend.tv_sec *1000000 + computationend.tv_usec)-(computationstart.tv_sec * 1000000 + computationstart.tv_usec));
