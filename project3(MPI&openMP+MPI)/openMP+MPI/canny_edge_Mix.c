...
int main(int argc, char **argv){
    ...
    int height,width,k_w,processors;
    //size_t write_ij;
    //FILE *fi;
    ...
    *h_chunk_feature,*v_chunk_feature;
  ...
  //printf("rank: %d processors: %d\n",comm_rank, comm_size); 
    //Horizonal gradient
   
    
    chunk_image_ghost = ghost_chunk(chunk_image, height/comm_size, width, floor(k_w/2) ,comm_size, comm_rank);
   convolve(chunk_image_ghost, &temp_horizon, gauss_Kernel, height/comm_size, width, k_w, 1, comm_size, comm_rank);
    chunk_image_ghost2 = ghost_chunk(temp_horizon, height/comm_size, width, floor(k_w/2),comm_size, comm_rank);
    
    
    convolve(chunk_image_ghost2, &h_temp_chunk, dgauss_Kernel, height/comm_size, width, 1, k_w, comm_size, comm_rank);
    MPI_Gather(h_temp_chunk, width*(height/comm_size), MPI_FLOAT, horizon,
               width*(height/comm_size), MPI_FLOAT, 0, MPI_COMM_WORLD);

    // Vertical gradient
    
    convolve(chunk_image_ghost, &temp_vertical, gauss_Kernel, height/comm_size, width,1, k_w, comm_size, comm_rank);
    chunk_image_ghost2V = (float*) malloc(sizeof(float)*width*(height/comm_size));
    chunk_image_ghost2V = ghost_chunk(temp_vertical, height/comm_size, width, floor(k_w/2),comm_size, comm_rank);
    convolve(chunk_image_ghost2V, &v_temp_chunk, dgauss_Kernel, height/comm_size, width, k_w, 1, comm_size, comm_rank);
    MPI_Gather(v_temp_chunk, width*(height/comm_size), MPI_FLOAT, vertical,
               width*(height/comm_size), MPI_FLOAT, 0, MPI_COMM_WORLD);
    
   
    // Magnitude
    
    Magnitude(v_temp_chunk, h_temp_chunk, &temp_Mag, height/comm_size, width);
    MPI_Gather(temp_Mag, width*(height/comm_size),MPI_FLOAT, Mag, width*(height/comm_size),
    MPI_FLOAT, 0, MPI_COMM_WORLD);
    
    // Direction
    Direction(v_temp_chunk, h_temp_chunk, &temp_Dir, height/comm_size, width);
    ...
    supression (&temp_sup, chunk_image_ghost_Mag, temp_Dir, width, height/comm_size, comm_rank, comm_size);
    ...
    hysteresis (temp_sup, &temp_hys, height/comm_size, width, t_high, t_low);
   

    
    MPI_Gather(temp_hys, width*(height/comm_size),MPI_FLOAT, hys, width*(height/comm_size),
    MPI_FLOAT, 0, MPI_COMM_WORLD);
    ...
    chunk_image_ghost_hys = ghost_chunk(temp_hys, height/comm_size, width, 1 ,comm_size, comm_rank);
        temp_edge = (float*)malloc(sizeof(float)*height/comm_size*width);
	memcpy(temp_edge, temp_hys, sizeof(float)*height/comm_size*width);
        Finaledge(&temp_edge, chunk_image_ghost_hys, height/comm_size, width, comm_size, comm_rank);
    ...
    feature_detec(height/comm_size, width, v_chunk_feature, h_chunk_feature, comm_size, comm_rank);
    ...
    if(comm_rank == 0){
        ...
        //print
        printf("Number of Threads: %d ",processors);
